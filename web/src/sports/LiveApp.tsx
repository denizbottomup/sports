...
function NewsCard({ item, saved, save, open }: { item: LiveNews; saved: boolean; save: () => void; open: () => void }) {
  return <article className={`news-card live-news ${item.image ? 'has-photo' : ''}`}>
    ...
    <div className="news-body"><div className="news-meta"><span className={`category ${item.official ? 'official' : 'press'}`}>{item.official ? 'Resmî açıklama' : 'Basın'}</span><span>·</span><time dateTime={item.publishedAt || undefined}>{fmt(item.publishedAt, item.datePrecision !== 'day')}</time>{item.reading?.language === 'pt' && <span className="language-pill">PT</span>}</div><button className="news-title" onClick={open}>{item.title}</button>{item.summary && <p>{item.summary}</p>}<div className="news-footer"><span>{item.source}</span><button className="read-news-button" onClick={open}>Haberi oku <ArrowRight size={14} /></button></div></div>
...
export default function LiveApp() {
  ...
  return <div className="sports-app live-app">
    ...
    {detail && <Modal title="Haber" close={() => setDetail(null)}>{detail.image && <Photo className="detail-photo" src={detail.image} fallback={detail.imageSource} alt={detail.title} />}<div className="detail-meta"><span className={`category ${detail.official ? 'official' : 'press'}`}>{detail.official ? 'Resmî açıklama' : 'Basın haberi'}</span><time>{fmt(detail.publishedAt, detail.datePrecision !== 'day')}</time></div><h2 className="detail-title">{detail.title}</h2><div className="article-reading">{(detail.reading?.paragraphs || [detail.summary]).map((p, index) => <p key={index}>{p}</p>)}</div><div className="detail-source"><ShieldCheck size={20} /><div><strong>Kaynak: {detail.source}</strong><small>{detail.reading?.kind === 'brief' ? 'Türkçe haber özeti · Touchline' : 'Kaynaktan kısa aktarım'} · {detail.reading?.language === 'pt' ? 'Portekizce' : 'Türkçe'}</small><small>Kaynak kontrolü: {fmt(detail.reading?.checkedAt || detail.firstSeenAt, true)}</small></div></div><div className="detail-actions"><button className="button secondary" onClick={() => toggle(detail)}><Bookmark size={16} />{saved.some(n => n.id === detail.id) ? 'Kaydı kaldır' : 'Kaydet'}</button><button className="button primary" onClick={() => setDetail(null)}>Akışa dön <ArrowRight size={16} /></button></div></Modal>}
    {info && <Modal title="Galatasaray ile başlıyoruz." close={() => setInfo(false)}><p className="detail-summary">Takımın Galatasaray olarak ayarlandı. Lig ve Avrupa'daki sıradaki rakipler, haberleri ve mevcut kadroları gerçek kaynaklardan geliyor.</p><div className="info-features"><p><Users size={22} /><span><strong>Giriş ve takım seçimi · İş planında</strong>Kayıt/giriş, ilk kullanımda takım seçimi, tercihlerin hesapta saklanması ve cihazlar arası eşitleme.</span></p><p><Radio size={22} /><span><strong>Haber akışı · Çalışıyor</strong>Resmî RSS 30 saniyede, basın dizini 60 saniyede kontrol edilir. Okunabilir metin ve başlık filtresi uygulanır. Kaynak keşfinden sonra açık sayfaya SSE ile iletilir. Kaynak gecikmesi bu süreye eklenebilir.</span></p><p><Activity size={22} /><span><strong>Taraftar nabzı · Sıradaki aşama</strong>Gerçek yorum erişimi ve yeterli örneklem olmadan yüzdeler gösterilmez. X/YouTube erişimi ve Türkçe çeviri ayrıca bağlanacak.</span></p><p><Bell size={22} /><span><strong>Bildirimler · İş planında</strong>Açık sayfa güncellenir. Tarayıcı kapalıyken push bildirimi henüz gönderilmez.</span></p></div><p className="source-disclosure">Haber kaydetme bu tarayıcıda tutulur. 7 lig ve UEFA organizasyonlarının tam kulüp kapsamı iş planında; bu sürüm Galatasaray'ın ilk lig ve Avrupa rakibini izler. Görsellerin hakları ilgili kulüp/yayıncıya aittir; kaynak adları haberlerde belirtilir.</p></Modal>}
  </div>;
}
